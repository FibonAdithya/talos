...
(`exhausted` and `Spend` must be in the file's `from talos.budget import ...` line.)
...
`measure_baseline`: before `resolve_baseline` (after the status save) set
...
pass `rule=self.rule`, and set `self.state.pending_job = None` right after it returns, before the status flips to `researching`.
...
Note for the implementer: `test_request_stop_cancels_the_job_and_raises_cancelled` replaces `b._run` after construction; that is the injected runner attribute, so keep the name `_run`.
...
**Files:**
- Modify: `talos/config.py`, `talos/cli.py`, `talos/agentic.py`, `tests/test_cli.py`, `tests/test_agentic.py`

**Interfaces:**
- Consumes: `C3Bench`, `ModalBench`, `FakeBench`, `PendingJobStore`, `EvalRequest`, `c3_hardware_class`, `c3_image`.
- Produces: `Config.backend: str = "modal"`; `cli.BACKENDS = ("modal", "c3")`; `cli.check_c3(run=None) -> float` (credit balance in GBP, raises `ConfigError` when not logged in); `cli.image_available(challenge, fetch=None) -> bool`; `cli.make_bench(backend, run_dir, pending) -> Bench`; `cli.MIRROR_HINT`; `cli.compile_backend(args, root) -> str`; environment variable `TALOS_BACKEND`, set by `execute_job` and passed through `agentic._agent_env`.

Why `TALOS_BACKEND` exists: agentic mode runs `talos compile` inside a temporary worktree created by `agentic.prepare_worktree` (a `tempfile.mkdtemp` outside the project), which has no `talos.config.json`. The compile command therefore resolves its backend from `--backend`, then `TALOS_BACKEND`, then the config in the current directory, then `modal`, and never fails on a missing config.
...
with `from talos.bench import EvalResult` added to the imports (`os` is already imported there). The three existing setup tests (`test_setup_writes_config_and_0600_secret`, `test_setup_rejected_key_writes_nothing`, `test_setup_cli_provider_stores_no_secret`) and `test_setup_rejects_the_fake_provider` answer a new first prompt, the backend: prepend `""` (accept the `modal` default) to each of their `scripted([...])` lists. `test_compile_refuses_an_empty_or_missing_dir` monkeypatches `talos.bench.ModalBench` with a `boom`; change it to `monkeypatch.setattr(cli, "make_bench", boom)`. Every `save(tmp_path, Config(...))` elsewhere keeps working because `backend` defaults to `"modal"`.
...
```python
    ...
    os.environ["TALOS_BACKEND"] = "modal" if fake else cfg.backend  # for `talos compile` in the sandbox
```

and the SIGINT handler becomes `lambda *_: (loop.request_stop(), bench.request_stop())`.

`cmd_compile`: resolve the backend without requiring a config, then evaluate with empty nonce sets:
...
and in `cmd_compile` after the file-map check:
...
`main()`: `c.add_argument("--backend", choices=list(BACKENDS))` on the compile parser. `talos/agentic.py`: add `"TALOS_BACKEND"` to `_AGENT_ENV_ALLOWLIST`, and note in the README's agentic section that on C3 each `talos compile` from the sandbox is one job of about 12 minutes. `import os` is already present in `talos/cli.py`? Check; add if not.
...
```bash
git add talos/config.py talos/cli.py talos/agentic.py tests/test_cli.py tests/test_agentic.py
...
(`PYTHON ?= python3` at the top of the Makefile means `make mirror-images PYTHON=.venv/bin/python` from a fresh shell, or activate the venv first; the README says so.)
